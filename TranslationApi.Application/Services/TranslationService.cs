...
namespace TranslationApi.Application.Services
{
    public class TranslationService : ITranslationService
    {
        ...
        private async Task<TranslationResponse> TranslateWithGeminiAsync(TranslationRequest request)
        {
            try
            {
                ...
                foreach (var chunk in textChunks)
                {
                    ...
                    var gemini = _aiModelService.GetModelByNameAndVersionAsync("Gemini", "2");
                    string apiUrl = $"{gemini.Result?.ApiEndpoint}?key={gemini.Result?.ApiKey}";
        ...
        public static string Serialize(object obj)
        {
            try
            {
                return JsonSerializer.Serialize(obj, JsonOptions.SafeOptions);
            }
            catch (Exception ex)
            {
        ...
        public static T Deserialize<T>(string json)
        {
            try
            {
                var result = JsonSerializer.Deserialize<T>(json, JsonOptions.SafeOptions);
                if (result == null)
                {
